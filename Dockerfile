...
RUN apk add --no-cache postgresql-libs postgresql-dev libxml2-dev libxslt-dev jpeg-dev zlib-dev bash apache2 apache2-dev apache2-ssl openssl && \
    apk add --no-cache --virtual .build-deps gcc musl-dev g++ make
...
COPY . /code/
COPY orperita.conf /etc/apache2/conf.d/
...
ENTRYPOINT dockerize -wait tcp://db:5432 /code/scripts/deploy.sh